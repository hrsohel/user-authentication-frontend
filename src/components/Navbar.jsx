                                    ...
                                    <div>
                                        {userData ? <p className='font-semibold underline'>Shops</p> : <></>}
                                        {
                                            userData && userData?.data?.shops.map((value, index) => {
                                                return <Link target='_blank' style={{marginLeft: "3px"}} className='block' key={index} to={`http://${value.split(" ").join("-")}.${import.meta.env.VITE_HO}?v=${state?.state?.crypted}`}>{value}</Link>
                                            })
                                        }
                                    </div>
